...
- Для настоящих ответов: Codex CLI с действующим входом пользователя. Baseline использовал **0.153.4**; при проверке 0.7 установлен **0.154.0-alpha.6.2**, schema пересоздана. Для обычной сборки и тестов CLI не нужен.
...
В 0.7 добавлены зависимости публичных EDT/BM/Xtext API и Jetty 10 из Target Platform EDT. Отдельный MCP-сервер устанавливать не нужно. Доступ открывается только на loopback и закрывается вместе с View. Для semantic integration tests требуется полная EDT.

### Проверки нативных инструментов 0.7

```powershell
.\scripts\start-dev-edt.ps1 -Smoke
.\scripts\start-dev-edt.ps1 -Smoke -SemanticLive
.\scripts\start-dev-edt.ps1 -Smoke -RestartPhase seed -RestartName semantic070 -SemanticRestart
.\scripts\start-dev-edt.ps1 -Smoke -RestartPhase restore -RestartName semantic070 -SemanticRestart
```

`-Smoke` выполняет semantic-тесты в настоящем продукте EDT. `-SemanticLive` явно включает настоящий Codex, создаёт одноразовый проект и программно нажимает разрешение/отказ только для заданного сценария. Он не предназначен для рабочей конфигурации. Restart использует один отдельный workspace между двумя запусками. Tycho пропускает native test вне полного продукта EDT и live tests без соответствующего флага.
...
В 0.7 smoke запускает полный продукт EDT с локально скомпилированными основным и тестовым JAR через `start-dev-edt.ps1`. Это необходимо для публичных сервисов Big Model. Используется отдельная одноразовая workspace, пользовательские проекты не открываются. Для чистой публикации без UI доступен параметр `-SkipSmoke`.
