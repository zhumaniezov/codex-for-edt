# Агентный режим 0.6–0.7

## Дополнение 0.7: семантические операции

Версия 0.6 принята пользователем; файловые режимы, approvals и diff сохранены. В 0.7 проект выбирается без открытого редактора. cwd существующего thread имеет приоритет; явная смена проекта начинает новый чат.

Новый native MCP вызывает публичную модель EDT внутри JVM. Он добавлен только в конфигурацию нашего thread, не в глобальный `config.toml`. «Только чтение» отвергает native запись. В строгом режиме и режиме запроса разрешения пользователь подтверждает весь Metadata Change Plan до BM-задачи. Другие write-режимы исполняют его в рамках явно выбранной политики. Native tools всегда ограничены текущим проектом, в том числе при Full Access.

Dirty guard остаётся обязательным. План выполняется транзакционно в отдельном editing context; `save(true)` сохраняет ресурсы средствами EDT. Невалидный тип или отсутствующий объект откатывает всю задачу. Существующий объект возвращается с `alreadyExists`. Клиент не создаёт metadata XML вручную, не применяет патчи поверх native модели и не имитирует Undo.

BSL-файлы после создания объекта меняет прежний App Server file agent с его approvals. Пустой BSL-модуль может ещё не иметь файла на диске; его первое содержимое записывается обычной файловой операцией в путь, выданный EDT. [API, мост и ограничения](edt-semantic-tools.md).

Установленный CLI к проверке 0.7 обновился до **0.154.0-alpha.6.2**, executable `%LOCALAPPDATA%/OpenAI/Codex/bin/bffc5354119c8421/codex.exe`. Schema вновь сгенерирована; `dynamicTools` не используются. Ниже сохранено исследование протокола исходной 0.6, выполненное на 0.153.4.
