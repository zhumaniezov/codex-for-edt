...
`org.eclipse.ui.views` регистрирует обычную перемещаемую View. SWT предоставляет многострочный ввод, кнопку Send и вывод. Асинхронный интерфейс клиента принимает обычные Java-данные; позднее вместо mock можно реализовать транспорт app-server. В текущей поставке сетевого транспорта, процессов Codex, авторизации и ключей нет.
