...
**Версия 0.7 — файловый агент Codex и нативные инструменты метаданных EDT.** Проект можно выбрать без открытого редактора. Справочники, общие модули, реквизиты и табличные части создаются через публичную модель EDT. Начальный режим остаётся «Только чтение»; запись включается явно.

Baseline — **`e37d291`**, версия 0.6 принята пользователем в настоящем BSL-редакторе. Lifecycle hardening и изоляция поздних событий сохранены. Точная первопричина исторического Error Part не объявляется доказанной. Новая semantic-функциональность требует ручной приёмки: [сценарии](docs/testing.md).

Среда: Windows x86_64, EDT **2026.1.3.25**, JDK **17**, Maven **3.9.16**, Tycho **4.0.5**. При работе над 0.7 установленный Codex обновился с **0.153.4** до **0.154.0-alpha.6.2**; schema пересоздана. Experimental API не включён.
...
- Создание справочника/общего модуля, добавление реквизитов и табличных частей; типы Строка, Число, Булево, Дата и СправочникСсылка.
