...
/**
 * Метод должен возвращаться без ожидания; завершение допускается на любом
 * потоке.
 */
public interface CodexClient extends AutoCloseable {
    ...
    default CompletionStage<String> send(ChatRequest request, Consumer<String> onText) {
        return send(request).thenApply(text -> {
            onText.accept(text);
            return text;
        });
    }
    ...
    default SessionData.Snapshot snapshot() {
        return SessionData.Snapshot.EMPTY;
    }

    default PermissionMode permissionMode() {
        return PermissionMode.READ_ONLY;
    }

    default PermissionOptions permissionOptions() {
        return PermissionOptions.CLOSED;
    }

    default CompletionStage<Void> selectPermission(PermissionMode mode) {
        return java.util.concurrent.CompletableFuture
                .failedFuture(new UnsupportedOperationException(tr("unavailable")));
    }

    default CompletionStage<Void> approve(String key, String decision) {
        return java.util.concurrent.CompletableFuture
                .failedFuture(new UnsupportedOperationException(tr("unavailable")));
    }
    ...
    default CompletionStage<SessionData.ThreadPage> threads(String cursor) {
        return java.util.concurrent.CompletableFuture
                .completedFuture(new SessionData.ThreadPage(java.util.List.of(), ""));
    }

    default CompletionStage<Void> newThread() {
    ...
    default CompletionStage<SessionData.HistoryPage> history(String cursor) {
        return java.util.concurrent.CompletableFuture
                .completedFuture(new SessionData.HistoryPage(java.util.List.of(), ""));
    }
    ...
    default CompletionStage<Void> interrupt() {
        return java.util.concurrent.CompletableFuture.completedFuture(null);
    }
    ...
    default CompletionStage<com.google.gson.JsonObject> manage(
    ...
    interface Listener {
        ...
        default void approvalResolved(String key) {
        }

        default void activity(AgentActivity.Snapshot activity) {
        }

        default void resourcesChanged(String directory) {
        }
