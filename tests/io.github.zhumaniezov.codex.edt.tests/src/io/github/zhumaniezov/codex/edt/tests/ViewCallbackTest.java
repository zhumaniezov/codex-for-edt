...
public class ViewCallbackTest {
    private static final class ControlledClient implements CodexClient {
        ...
        public CompletionStage<ConnectionInfo> connect() { return new MockCodexClient().connect(); }
        ...
        public CompletionStage<String> send(ChatRequest request, Consumer<String> text) {
            var future = new CompletableFuture<String>(); answers.add(future); consumers.add(text); return future;
        }
        public void close() { }
    }

    @Test public void lateUiCallbackCannotEnterNextResponseOrDisposedView() throws Exception {
        ...
        try {
            ...
            prompt.setText("Вопрос B"); waitFor(send::isEnabled, 10, () -> { });
            send.notifyListeners(SWT.Selection, new Event()); waitFor(() -> client.consumers.size() == 2, 5, () -> { });
            client.consumers.get(0).accept("STALE_A"); client.consumers.get(1).accept("ONLY_B");
            client.answers.get(1).complete("ONLY_B");
            waitFor(() -> response.getText().contains("ONLY_B"), 10, () -> { });
            assertTrue(response.getText().contains("PARTIAL_A")); assertFalse(response.getText().contains("STALE_A"));
            client.consumers.get(1).accept("QUEUED_AFTER_COMPLETION");
            page.hideView(view); client.consumers.get(0).accept("AFTER_DISPOSE");
            while (shell.getDisplay().readAndDispatch()) { }
            assertTrue(response.isDisposed());
        } finally { page.hideView(view); registration.unregister(); }
    }
}
